...
**Research system** (2026-09-05, 主題研究): **one deck slide per research theme, no detail pages** (user removed them all after seeing them — links go straight out instead). Deck order (content.ts `slideOrder`): hero, about, the four theme slugs, the four lesson slugs, join — 11 slides. Each research slide: title + one-liner + 領域標籤 chips + external actions + the visual, whose treatment depends on `visuals[slug].mode` — a **key visual** (`background`) is blown up as a large slide-background layer with a left-heavy scrim (VE challenge visual, CDGRN concept diagram); a **logo** (`figure`) stays a framed figure beside the text (GeometricFlux, RAFAEL — logos are never slide backgrounds, user's explicit call): `externalCta` renders a button straight to the site (VE → challenge site, GeometricFlux → GitHub); `links` render as a separate row of small links **below** the button (CDGRN: Paper + GitHub — never on the same row as a button, user's explicit call); 聯邦式學習 (RAFAEL presented by field name) has neither. **No related-publication lists on slides** (user's explicit call — VE/FL have no paper; only CDGRN links its Paper). Slide washes alternate leaf/sky (`#virtual-embryo,#federated-learning` = leaf, `#geometricflux,#cdgrn` = sky). Header nav: single 研究 entry covering all four slides (`data-nav-group` → `#virtual-embryo`). Data: `src/data/research.ts` (`ResearchTheme` + `themeImages` static import map + `visuals` alt text; `desc`/`metrics` are kept-but-unused for future use). `ResearchPage.astro` was deleted; the former projects slide, publications slide, interests grid, focus chips, `projects`/`interests`/`focusChips` are all gone from content.ts (`publications` stays as the single pub source). Full publication list: `src/components/PublicationsPage.astro`, deployed at the blog ROOT `/publications/` (+`/en/publications/`) — the workflow copies it out of the `/lab` build; the 論文 nav item is a **page link** (root-absolute) (never glows on the deck; pre-lit via Header's `activeNav` prop). Visuals: VE key visual at `src/assets/challenge/virtual-embryo-challenge.png` (user-supplied); GeometricFlux logo (MIT), RAFAEL logo and the CDGRN concept figure (rasterized from the user's diagrams.net SVG via rsvg-convert — Astro can't parse diagrams.net SVGs) in `src/assets/research/`.
