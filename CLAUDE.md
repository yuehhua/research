...
**Research system** (2026-09-05, 主題研究): **one deck slide per research theme** (same pattern as lessons — the 2×2 card overview was broken down too). Deck order (content.ts `slideOrder`): hero, about, the four theme slugs, the four lesson slugs, join — 11 slides. Each research slide: title + one-liner + 領域標籤 chips + CTA to its detail page + the representative visual as a **framed figure** (right column; logos/figures are never used as slide backgrounds). Slide washes alternate leaf/sky by position (`#virtual-embryo,#federated-learning` = leaf, `#geometricflux,#cdgrn` = sky). Header nav: single 研究 entry covering all four slides (`data-nav-group`, links to `#virtual-embryo`). The former projects slide, publications slide, interests grid and focus chips were **removed** (user's explicit calls) — `projects`/`interests`/`focusChips` no longer exist in content.ts; `publications` stays there as the single pub source. Data: `src/data/research.ts` (`ResearchTheme`: slug, accent, name, status, short, desc, metrics, links, tags, **pubDois** — related pubs resolve against content.ts publications by DOI; `themeImages` static import map). Detail pages: `src/components/ResearchPage.astro` (figure-framed visual, desc, metrics, tags, related pubs) at `src/pages/<slug>.astro` + `src/pages/en/<slug>.astro` for slugs `virtual-embryo`, `geometricflux`, `federated-learning` (RAFAEL presented by its field name 聯邦式學習), `cdgrn`; their back-links return to the theme's own deck slide (`#<slug>`). Full publication list: `src/components/PublicationsPage.astro` at `/publications/` (+/en/); the 論文 nav item is a **page link** (never glows on the deck; pre-lit via Header's `activeNav` prop on that page). Visuals live in `src/assets/research/` — VE task2.webp from the challenge site (credited on the page), GeometricFlux logo (MIT), RAFAEL logo, CDGRN concept figure rasterized from the user's own diagrams.net SVG (rsvg-convert; Astro cannot parse diagrams.net SVGs).
