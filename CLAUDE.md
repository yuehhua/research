...
ISB Lab (Intelligent Systems Biology Lab, 臺北醫學大學) academic website. A static Astro site doubling as a presentation deck: full-viewport scroll-snap sections act as slides. Bilingual (zh default, en). Live at <https://yuehhua.github.io/lab/> (moved from `/research/` on 2026-09-05; the publication list lives at the blog ROOT, <https://yuehhua.github.io/publications/>, outside the base path)

Source repo: **github.com/yuehhua/research** (push to `main` → GitHub Actions builds and publishes `dist/` to the `lab/` directory of the `yuehhua.github.io` repo, additionally copies `dist/publications` → `/publications/` and `dist/en/publications` → `/en/publications/` at the blog root, and wipes the legacy `research/` directory; requires the `DEPLOY_TOKEN` secret). Do not commit directly to the blog repo.

## Commands

```bash
npm run dev          # dev server (note: base path means it serves under /lab/)
...
**Research system** (2026-09-05, 主題研究; AI Agent theme added 2026-09-07): **one deck slide per research theme, no detail pages** (user removed them all after seeing them — links go straight out instead). Deck order (content.ts `slideOrder`): hero, about, the five theme slugs (`virtual-embryo, ai-agent, cdgrn, geometricflux, federated-learning`), the four lesson slugs, join — 12 slides. Each research slide: title + one-liner + 領域標籤 chips + external actions + the visual, whose treatment depends on `visuals[slug].mode` — a **key visual** (`background`) is blown up as a large slide-background layer with a left-heavy scrim (VE challenge visual, CDGRN concept diagram); a **logo** (`figure`) stays a framed figure beside the text (GeometricFlux, RAFAEL — logos are never slide backgrounds, user's explicit call); **`none`** is a text-only slide with no image at all (available for future themes; not currently used): `externalCta` renders a button straight to the site (VE → challenge site, GeometricFlux → GitHub); `links` render as a separate row of small links **below** the button (CDGRN: Paper + GitHub — never on the same row as a button, user's explicit call); 聯邦式學習 has neither. **No related-publication lists on slides** (user's explicit call — only CDGRN links its Paper). Slide washes alternate leaf/sky by position, not by theme identity — re-derive per slide order rather than hardcoding a fixed leaf/sky split when reordering or inserting themes (currently `#virtual-embryo,#cdgrn,#federated-learning` = leaf, `#ai-agent,#geometricflux` = sky). Header nav: single 研究 entry covering all theme slides (`data-nav-group`, built from `researchThemes.map(r => r.slug)` — new themes are picked up automatically, → `#virtual-embryo`). Data: `src/data/research.ts` (`ResearchTheme` + `themeImages` static import map + `visuals` alt text/mode; `desc`/`metrics` are kept-but-unused for future use — AI Agent has `metrics: []`). `ResearchPage.astro` was deleted; the former projects slide, publications slide, interests grid, focus chips, `projects`/`interests`/`focusChips` are all gone from content.ts (`publications` stays as the single pub source). Full publication list: `src/components/PublicationsPage.astro`, deployed at the blog ROOT `/publications/` (+`/en/publications/`) — the workflow copies it out of the `/lab` build; the 論文 nav item is a **page link** (root-absolute) (never glows on the deck; pre-lit via Header's `activeNav` prop). Visuals: VE key visual at `src/assets/challenge/virtual-embryo-challenge.png` (user-supplied); GeometricFlux logo (MIT), RAFAEL logo and the CDGRN concept figure (rasterized from the user's diagrams.net SVG via rsvg-convert — Astro can't parse diagrams.net SVGs), plus a hand-drawn `ai-agent-mascot.svg` (Claude-authored, plain SVG — Astro parses it directly, no rasterization needed; a waving robot connected to memory/discovery/teaching icon-nodes) all in `src/assets/research/`.
...
**Deployed under a base path** (`base: '/lab'` in astro.config.mjs). The publications page is the one route that lives OUTSIDE the base path (blog root `/publications/` + `/en/publications/`) — its nav entry and canonical are hardcoded root-absolute. All internal links must go through the `base` variable pattern used in each component (`(import.meta.env.BASE_URL === '/' ? '' : import.meta.env.BASE_URL).replace(/\/$/, '')`), never a raw `/href`.
...
- **Highlights ordered first**: the `researchThemes` array order in research.ts is deliberate (Virtual Embryo → GeometricFlux.jl → 聯邦式學習 → CDGRN). Theme cards carry **no serial numbers and no separator rules** — whitespace separates (user preference).
...
⚠️ The blog repo is Hexo-deployed by the user. If they run `hexo deploy`, the `lab/`, `publications/` and `en/` directories may be wiped (Hexo cleans non-`public/` files) — the fix is `keep_files: lab publications en` in the blog's `_config.yml` deploy section, or re-run this repo's workflow (`gh workflow run deploy.yml --repo yuehhua/research`).
